...
if __name__=='__main__':
    ...
    if args.sim:
        args.file = 'data/varying_freq_'+str(round(args.freq))+'.npy'
        run_simulation_with_input(args)
    else: # plot
        import matplotlib.pylab as plt
        fig, ax = plt.subplots(2)
        import os
        freqs = [1, 2.5, 5, 10, 25, 50, 100, 250]
        for file in os.listdir("./data/"):
            split = file.split("varying_freq_")
            if (len(split)>1):
                if (float(split[1].split('.npy')[0]) in freqs):
                    ...
                    ax[0].plot(t_th, .8*fe_th+.2*fi_th, 'k-', lw=3, alpha=.5)
                    t_th *=1e3
                    ax[0].plot(t_th[t_th>300], .8*fe_th[t_th>300]+.2*fi_th[t_th>300], 'k-', lw=3, alpha=.5)
        plt.show()
