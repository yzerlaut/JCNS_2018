...
On the other hand, the litterature lacks, to the best of our
...
generated and those physiolgical phenomena at that spatial scale
...
cortical column and 2) we embeded the analytical description of this
...
spatial description with a translational invariance, i.e. for all
...
choosen as a piecewise double Gaussian waveform:
...
single neurons [[cite:VanVreeswijk1996]].  Recurent neural networks
...
estimates [[REFS?]]. Importantly, we integrate distance-dependent
...
an input (see Equation [[eq:ring-input]]). The spatio-temporal parameters
of the input (\(\tau_1\)=50ms, \(\tau_2\)=150ms, \(l_{exc}\)=1.5mm)
have been manually calibrated to reproduce the properties of the
response observed in /in vivo/ as a response to a single flash (see
next section). In space, the profile was a Gaussian curve, in time, it
was a piecewise double Gaussian function. Despite its various
amplitude over space, it should be emphasized that this input does not
propagate: e.g. its maximum is achieved at all position at the same
time. To highlight this feature, we implemented a simple analysis of
propagation: we normalize the responses with respect to their local
amplitude and we look for a specific crossing of the normalized
amplitude. To focus on early responses, we highlight the initial
crossing of the fourth of the normalized amplitude over space, we call
it the /early response line/ (drawn with a white dashed-line, see
Figure [[fig:simple-stim]]). In Figure [[fig:simple-stim]]A, the horizontal
/early response line/ indeed show that the input does not propagate,
the fourth of the maximum of the normalized response is achieved
...
and Table [[table:params]]), the excitation has broad spatial extent, it
...
nonethless exhibits a very strong attenuation over space, this is due
...
** Comparison with VSDi response in 

#+ATTR_LATEX: :float multicolumn :width .8\linewidth
#+NAME: fig:suppression
#+CAPTION: *Apparent motion protocol and the associated spatio-temporal profile of suppression: comparison between the model and VSDi recordings in an awake behaving monkey.* *(A)* The apparent motion protocol. *(B)* VSDi in the primary visual cortex in vivo. *(C)* COmparison between model and experiments.
[[../figures/suppression.png]]

We now have a model that exhibits the strong propgation effect
observed in VSDi recordings during the presentation.

VSDi allows for a real-time visualization of large neuronal population
in V1 of fixating monkeys
...
shown to be an relatively accurate description of the response
simulated in numerical networks, we highlighted two example phenomena
that was missed by our description. The relative complexity of the
...
thalamic nucleus, thus ruling out the hypothesis of synaptic
