...
#+CAPTION: *Limitations of the Markovian description in the frequency domain.* Response of the network (numerical simulation and analytical description) to sinusoidal stimulation of the form \(\nu_e^{aff} = 5\mathrm{Hz} \, \big(1-\cos(2 \pi f (t-t_0))\big)/2\). The stimulation was set on at \(t_0\)=500ms. *(A)* Amplitude of the sinusoidal response for various frequencies. *(B)* Phase shift of the sinusoidal response for various frequencies.
[[../figures/freq_dep_of_network.png]]


To study more precisely the temporal validity of the formalism, we
modulated the network activity by sinusoidal input (i.e. sinusoidal
presynaptic firing rates that were then translated into individual
afferent spikes)

The main input of this is to show that the network react very fast
(compared to the scale of VSDi) and that the model accurately
reproduce this feature.

The network time constant would be
...
transient effects. We highlighted two example phenomena that l

The hyperpolarization (unpublished observations)
