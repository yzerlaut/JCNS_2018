#+TITLE: A mean-field model for spatio-temporal integration in neocortex
#+AUTHOR:  Zerlaut Y., Chemla S., Chavane F., Destexhe A.
...
signal at the spatial scale imaged by optical imaging (\(\sim\)
100mm^2). In this communication, we take advantage of the extensive
theoretical work performed at the mesoscopic scale (\(\sim\) 10000
neurons, 0.25mm^2) to analytically describe population dynamics in
terms of few variables (the so-called /mean-field/ approach to network
dynamics) to build a large scale model with a correlate in terms of
single-cell dynamics so that the model can directly generate
predictions for the signal imaged by the VSDi technique. Our study
proposes such a model and examine its predictive power by comparison
with VSDi recordings in the primary visual cortex of fixating monkey.

The first part of our study recapitulates the strengths and weaknesses
of the mesoscopic description of network dynamics used to model the
activity of a single cortical column. We campare the mean-field
predictions with numerical simulations of artificial neural networks,
we investigate whether the markovian formalism proposed in
[[citetext:ElBoustani2009]] allows to describe the dynamics of a network
of excitatory and inhibitory neurons with different
electrophysiological properties. In particular, we investigate whether
this formalism generalizes to the description of network dynamics in
the presence of time-dependent inputs.
