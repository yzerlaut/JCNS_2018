...
other network studies [[cite:Brunel2000,Vogels2005,Kumar2008]]

The reason for putting this assymetry in the excitabilities was

exactly to produce an inhibitory control of the mean membrane
potential flcutuations that would give rise to such a strong
suppression.
...
An important constraint was that the strong suppressive mechanism
would be cortico-cortical, indeed depression at the thalamo-cortical
synapses can contribute to the suppression of cortical responses [[ref]]

Future directions for the present model includes: stimulus response
dependency to the level of ongoing activity [[cite:Arieli1996]] dependency
on the level of ongoing activity

Improving the theoretical descriptions of the local cortical network,
for example the hyperpolarization can be described by adding a
variable: the "population spike-frequency adaptation current" (not
shown).
